...
export const useSlotCallStore = create<SlotCallState>((set, get) => ({
	...
	addSlotCall: async (slotName, betAmount) => {
		...
		try {
			const res = await fetch(
				"https://mokingdata-ehej.onrender.com/api/slot-calls",
				{
					method: "POST",
					headers: {
	...
	updateSlotStatus: async (id, status) => {
		...
		try {
			const res = await fetch(
				`https://mokingdata-ehej.onrender.com/api/slot-calls/${id}/status`,
				{
	...
	fetchSlotCalls: async () => {
		...
		const url =
			userRole === "admin"
				? "https://mokingdata-ehej.onrender.com/api/slot-calls"
				: "https://mokingdata-ehej.onrender.com/api/slot-calls/my";
